"""SparkTestSession — test utility for local PySpark + Delta sessions.

Centralises the setup boilerplate that every test conftest would otherwise
duplicate:

* Auto-discovers ``JAVA_HOME`` from common installation paths when not set.
* Configures the Delta Lake SQL extensions in one place.
* Provides a ``skip_if_unavailable()`` helper so tests are skipped cleanly
  in environments without a JVM (e.g. lightweight CI stages).
