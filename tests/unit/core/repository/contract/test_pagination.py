...
async def test_offset_pages_cover_every_row_once(case: BackendCase, seeded: Any) -> None:
    require(case, Listable)

    seen: list[int] = []
    for page in range(1, len(SEED) // _PAGE + 1):
        seen.extend(row.id for row in (await _page(seeded, page)).items)

    assert seen == oracle(seed_rows())


async def test_offset_total_reflects_the_filter(case: BackendCase, seeded: Any) -> None:
    require(case, Listable)
    expected = oracle(seed_rows(), _PAID)

    first = await _page(seeded, 1, _PAID)
    beyond = await _page(seeded, len(expected) // _PAGE + 2, _PAID)
