...
class TestKafkaWireDecode:
    def test_try_decode_record_maps_envelope_to_stream_message(self) -> None:
        codec = MsgspecCodec[_OrderCreated]()
        envelope = build_message(
            _OrderCreated(order_id="o-1", amount=42),
            MessageDescriptor(message_type="order.created", message_version=2),
        ...
        result = try_decode_record(record, _OrderCreated, codec)

        assert isinstance(result, DecodeOk)
        assert result.message.payload == _OrderCreated(order_id="o-1", amount=42)
        ...
        assert result.message.meta.message_version == 2
