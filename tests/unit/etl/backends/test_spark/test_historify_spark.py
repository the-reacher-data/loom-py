"""Unit tests for SparkHistorifyEngine — SCD Type 2 with real Delta tables."""
...
from loom.etl.backends.spark._historify import (  # noqa: E402
...
from loom.etl.storage.routing import PathTarget  # noqa: E402
...
def _path_target(spark_root: Path, name: str = "dim_players") -> PathTarget:
    path = spark_root / name
    path.mkdir(parents=True, exist_ok=True)
    return PathTarget(
        logical_ref=TableRef(f"wh.{name}"),
        location=TableLocation(uri=str(path)),
    )


def _eng() -> SparkHistorifyEngine:
    return SparkHistorifyEngine()


def _read(spark: SparkSession, target: PathTarget) -> list[dict[str, Any]]:
    return [row.asDict() for row in spark.read.format("delta").load(target.location.uri).collect()]
...
class TestBootstrap:
    def test_creates_table(self, spark: SparkSession, spark_root: Path) -> None:
        target = _path_target(spark_root)
        frame = spark.createDataFrame(
            [{"player_id": 1, "team_id": "RM"}, {"player_id": 2, "team_id": "BCA"}]
        )
        _eng().apply(spark, frame, target, _snapshot_spec(), _Params(run_date=date(2024, 1, 1)))
        assert len(_read(spark, target)) == 2
...
class TestNoOp:
    def test_no_new_rows_when_unchanged(self, spark: SparkSession, spark_root: Path) -> None:
        ...
        frame = spark.createDataFrame([{"player_id": 1, "team_id": "RM"}])
        eng = _eng()
        eng.apply(spark, frame, target, _snapshot_spec(), _Params(run_date=date(2024, 1, 1)))
        eng.apply(spark, frame, target, _snapshot_spec(), _Params(run_date=date(2024, 1, 2)))
        rows = _read(spark, target)
        assert len(rows) == 1
...
class TestLoanCase:
    def test_two_vectors_open_simultaneously(self, spark: SparkSession, spark_root: Path) -> None:
        ...
        _eng().apply(spark, frame, target, spec, _Params(run_date=date(2024, 1, 1)))
        rows = _read(spark, target)
        assert len(rows) == 2
        assert all(r["valid_to"] is None for r in rows)
...
class TestIdempotency:
    def test_same_day_rerun_stable_row_count(self, spark: SparkSession, spark_root: Path) -> None:
        ...
        eng.apply(spark, frame2, target, _snapshot_spec(), _Params(run_date=date(2024, 6, 1)))
        eng.apply(spark, frame2, target, _snapshot_spec(), _Params(run_date=date(2024, 6, 1)))
        assert len(_read(spark, target)) == 2
...
class TestLogBootstrap:
    def test_events_get_valid_from(self, spark: SparkSession, spark_root: Path) -> None:
        target = _path_target(spark_root, "dim_subs")
        frame = spark.createDataFrame(
            [
        ...
        _eng().apply(spark, frame, target, _log_spec(), None)
        rows = sorted(_read(spark, target), key=lambda r: r["valid_from"])
