...
def test_build_lazy_frame_handles_empty_and_rows() -> None:
...
def test_polars_capturing_writer_stores_last_call() -> None:
...
def test_polars_stub_reader_resolves_table_ref_or_alias() -> None:
...
def test_polars_step_runner_guards_before_run() -> None:
...
def test_polars_step_runner_run_returns_step_result_and_resolved_predicate(
    ...
    def _fake_run_step(self: ETLExecutor, _plan: Any, params_instance: _RunParams) -> None:
        predicate = col("day") == params.run_day
...
def test_polars_step_runner_run_raises_when_no_output(monkeypatch: pytest.MonkeyPatch) -> None:
    runner = PolarsStepRunner().seed("raw.orders", [(1, 10.0)], ["id", "amount"])
...
def test_step_result_assertions_and_show(capsys: pytest.CaptureFixture[str]) -> None:
...
def test_step_result_failures_raise_assertion_error() -> None:
    result = StepResult(pl.DataFrame({"id": [1], "amount": [10.0]}))
