"""Install a third-party MCP auth strategy the way a real distribution would.

The point of ``loom.ai.mcp_auth`` is that someone who is not loom can register a
strategy. Monkey-patching the loader would prove nothing about that, so this
helper writes a genuine distribution — a module plus its ``.dist-info`` with an
...
STRATEGY_MODULE = "third_party_mcp_auth"
"""Module name of the installed strategy package."""

STRATEGY_SOURCE = dedent(
    '''
    """A deployment's own MCP authentication, living outside loom."""
...
def third_party_strategy(root: Path, *, name: str) -> Iterator[None]:
    """Register ``name`` in ``loom.ai.mcp_auth`` from a distribution outside loom.
